...
class Zomato:
    ...
    def restaurant_search(self, query="", latitude="", longitude="", cuisines="", sortby="", orderby="", start=0, limit=20):
        """
        Takes either query, latitude and longitude or cuisine as input.
        Returns a list of Restaurant IDs.
        """
        print("Calling Zomato with start:  ", start)
        cuisines = "%2C".join(cuisines.split(","))
        if str(limit).isalpha() == True:
            raise ValueError('LimitNotInteger')
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        r = (requests.get(base_url + "search?q=" + str(query) + "&start=" + str(start) + "&count=" + str(limit) + "&lat=" + str(latitude) + "&lon=" + str(longitude) + "&cuisines=" + str(cuisines) + "&sort=" + str(sortby) + "&order=" + str(orderby), headers=headers).content).decode("utf-8")
