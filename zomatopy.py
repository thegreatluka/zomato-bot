...
class Zomato:
    ...
    def restaurant_search(self, query="", latitude="", longitude="", cuisines="", sortby="", orderby="", limit=5):
        """
        ...
        if str(limit).isalpha() == True:
            raise ValueError('LimitNotInteger')
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        r = (requests.get(base_url + "search?q=" + str(query) + "&count=" + str(limit) + "&lat=" + str(latitude) + "&lon=" + str(longitude) + "&cuisines=" + str(cuisines) + "&sort=" + str(sortby) + "&order=" + str(orderby), headers=headers).content).decode("utf-8")
